...
public class FloatingCameraWindow {
    ...
    public FloatingCameraWindow(Context context) {
        ...
        mWindowWidth =
                mWindowWidth > 0 && mWindowWidth < mScreenMaxWidth
                        ? mWindowWidth
                        : mScreenMaxWidth;
        mWindowHeight =
                mWindowHeight > 0 && mWindowHeight < mScreenMaxHeight
                        ? mWindowHeight
                        : mScreenMaxHeight;
    }
    ...
    public FloatingCameraWindow(Context context, int windowWidth, int windowHeight) {
        ...
        if (windowWidth < 0 ||
                windowWidth > mScreenMaxWidth ||
                windowHeight < 0 ||
                windowHeight > mScreenMaxHeight) {
    ...
    private void init() {
        mUIHandler.postAtFrontOfQueue(new Runnable() {
            @Override
            public void run() {
                Log.v("test27", "init() called for FloatingCameraWindow");
    ...
    private WindowManager.LayoutParams initWindowParameter() {
        ...
        mWindowParam.flags =
                mWindowParam.flags | WindowManager.LayoutParams.FLAG_WATCH_OUTSIDE_TOUCH;
        mWindowParam.flags =
                mWindowParam.flags | WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS;
    ...
    private final class FloatCamView extends FrameLayout {
        ...
        public FloatCamView(FloatingCameraWindow window) {
            ...
            mLayoutInflater =
                    (LayoutInflater) window.mContext
